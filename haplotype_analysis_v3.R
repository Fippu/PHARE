...
setwd("C:/Users/Philipp/Documents/SNPs")


bc_to_sample = read_excel("pf_control_experiment_221124.xlsx", col_types = 'numeric') %>%
  arrange(
...
init_phare = function(
    ...
    pattern = '*.tsv',
    samples, # a DATA FRAME! with one column containing `Source` (== barcode) and another titeled `Sample`
    fasta_ref,
    gb_report,
    gene_name,
    threshold,
  ...
  phare$gb_rep = fromJSON(file=gb_report)
...
read_folder = function(path, pattern = '*.tsv') {
  ...
  for (i in 1:length(file_list)){
    source = str_extract_all(file_list[i], "\\d+")[[1]]
...
make_hapl_df = function(phare){
  ...
  phare$df = hapl %>%
    ...
    mutate(
      Sample_name = factor(Sample, levels=levels(phare$samples$Sample)),
...
phare_translate = function(phare){
  ...
  for (snp_i in 1:nrow(phare$snps)){
    ...
    aa_col = paste(snp$aapos, 'aa', sep='_')
...
# tmp3 = copy(tmp2) %>% phare_translate()
...
apply_threshold = function(phare) {
  ...
  for (snp_i in 1:nrow(snps)){
    ...
    snps[snp_i, 'wt'] = wt_aa
    if (length(variants) == 1){ if (variants == wt_aa){
...
phare_plot = function(phare){
  ...
  p1 = (
    ...
    + scale_y_continuous(breaks=sort(unique(as.numeric(hapl$Sample_name)))*1.2-.55,
                         labels=(summarize(group_by(hapl[c('Sample_name', 'total_over_threshold')], Sample_name), tot=max(total_over_threshold)) %>% mutate(lab = paste(as.character(Sample_name), tot, sep='\nn=')))$lab,
                         position = 'left',)
...
minimap_plot = function(phare){
  hapl = phare$df
  
  (
    ggplot(
      filter(hapl, aa_hapl_plot == 'SA'),
      aes(
        y = ifelse(freq_plot<0, 0, freq_plot*100),
        x=minimap_ratio
      )
    )
    + geom_point(aes(
...
get_small_df = function(phare){
  phare$df %>% select(-paste(phare$snps$aapos, 'reftrip', sep='_'), -as.character(phare$snp$snp))
}
