...
# if the output of this loop is cached, read cache
if not path.isfile(infile + '.res' + '.json'):
    ...
    snpsflat = snpsfiltered = [snp for exon in snps for snp in exon['snps']]
...
# read cached file
    ...
    with open(infile + '.res' + '.json') as f:
        rescache = json.load(f)
        res = rescache['res']
        snpsflat = rescache['snps']
    snpsfiltered = rawsnps
...
nofail = suc.query(f'not `{ "` and not `".join(map(str, snpsfiltered)) }`').index
...
trip = trip.loc[nofail, map(str, snpsfiltered)]
...
amino = amino.loc[nofail, map(str, snpsfiltered)]
...
# amino.value_counts().rename('count').to_csv(outfile, sep='\t')


if makesubsets:
    for perc in makesubsets:
        for repl in range(0, replicates):
            trues = perc #round(len(trip)*(perc/100)) 
