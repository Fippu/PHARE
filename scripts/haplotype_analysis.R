...
read_folder = function(path, pattern = '*.tsv') {
  ...
  for (i in 1:length(file_list)){
    source = str_extract_all(file_list[i], "(?<=[_|\\.])\\d+")[[1]]
...
save_plot = function(phare, width=320, height=180, sup='phare'){
  ggsave(sprintf('%s_%s_%s.png', phare$gene, phare$threshold, sup), phare$fig, width=width, height=height, units='mm')
  phare
}
